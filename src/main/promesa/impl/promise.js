// Logsidian override of promesa 11.0.678 promesa/impl/promise.js (pinned in
// deps.edn, deps/common, deps/graph-parser and deps/shui). It shadows the
// jar's copy because src/main comes first on the classpath.
//
// Why: processNextTick drained a promise's queue with `let task, rvalue,
// rcause;` declared once and never reset, so after one handler threw, every
// later task in the same drain completed with that stale error. Every p/let
// and p/do! starts from the shared NULL_PROMISE (resolved(null)), so
// unrelated chains started in the same tick share a drain: one chain's
// synchronous throw rejected the others (seen as a test receiving another
// test's error, and catches that "did not catch"). The only change from the
// upstream file is the per-task reset marked "Logsidian patch" below.
// Remove this file when promesa is upgraded to a release with the fix
// (regression test: src/test/frontend/promesa_drain_test.cljs).
"use strict";
...
goog.scope(function() {
  ...
  function processNextTick(p) {
    ...
    while (p[QUEUE].length) {
      task = p[QUEUE].shift();
      // Logsidian patch: reset per task. Upstream declares these once per
      // drain and never clears rcause after a handler throws, so every later
      // task in the same drain was rejected with that stale error.
      rvalue = undefined;
      rcause = undefined;
