...
## Update 2026-09-11 — premise corrected (see ADR-003)

The original text above is left unchanged. One premise in it does not hold:

- **"Lazy loading via IStorage (JVM-only feature)"** (Proposed Benefits) is wrong
  for this tree. The pinned ClojureScript DataScript fork (`logseq/datascript`
  3971e2d) implements `IStorage`, and the db worker already uses it for every
  repo, file graphs included: `restore-data-from-addr` performs a synchronous
  sqlite-wasm `.exec` on an OPFS SAH-pool database
  (`src/main/frontend/worker/db_worker.cljs:158-192`), and
  persistent-sorted-set 0.1.2 restores B-tree children on demand behind
  `js/WeakRef` (`persistent_sorted_set.cljs:330-336,429-445`).
- The eager, partial database this ADR describes is the **UI thread's** copy,
  built with `d/conn-from-datoms` from initial data
  (`src/main/frontend/db/restore.cljs:18-24`), not the worker's.

The rejection of the thin-worker design itself still stands for the other
reasons given here: a partial entity set breaks `d/entity` reference chains,
reactive refresh and undo (which lives on the main-thread connection,
`src/main/frontend/undo_redo.cljs:174-252`). ADR-003 records what replaces it.
